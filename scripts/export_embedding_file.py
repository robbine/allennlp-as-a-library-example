...
def parse_args():
    ...
    parser.add_argument('--weights-file-name', help='weigth file name', default='best.th')
    parser.add_argument('--embedder-name', help='embedder name', default='_text_field_embedder.token_embedder_tokens.weight')
    parser.add_argument('--vocab-file-folder', help='vocab file folder', default='vocabulary')
    parser.add_argument('--output-embedding-file', help='output embedding file name', default='exported_embedding.tar.gz')
...
def load_weights(serialization_dir, weights_file_name, embedder_name):
    ...
    print(type(embedder_weight))
    return embedder_weight.data.cpu().numpy()[1:, :]

def load_vocab(serialization_dir, vocab_file_folder):
    vocab_file_path = os.path.join(serialization_dir, vocab_file_folder, _TOKENS)
    tokens = []
...
def save_embedding_file(weights, tokens, serialization_dir, output_embedding_file):
    embedding_fn = os.path.join(serialization_dir, 'exported_embedding.txt')
    with open(embedding_fn, 'w') as f:
        size, dim = weights.shape
        f.write('{} {}\n'.format(size, dim))
        for i in range(size):
            f.write('{} {}\n'.format(tokens[i], ' '.join([str(s) for s in weights[i].tolist()])))
    with tarfile.open(os.path.join(serialization_dir, output_embedding_file), 'w:gz') as tar_file:
