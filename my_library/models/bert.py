...
class Bert(Model):
    ...
    def partial_half(self):
        ...
        for name, module in self.named_children():
            print(name)
            if isinstance(module, nn.Linear) == False and isinstance(
                    module, nn.LayerNorm) == False:
                module._apply(fn)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        return {
            'accuracy':
            self._masked_lm_accuracy.get_metric(reset),
            'next_sentence_accuracy':
            self._next_sentence_accuracy.get_metric(reset)
        }


def get_masked_lm_output(use_fp16, input_tensor, norm_layer, bias,
                         masked_lm_feedforward, output_weights, positions,
    ...
    if label_ids.is_cuda:
        one_hot_labels = torch.cuda.FloatTensor(
            label_ids.size(0),
            vocab_size,
            device=util.get_device_of(label_ids))
    else:
