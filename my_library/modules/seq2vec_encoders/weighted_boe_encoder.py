...
@Seq2VecEncoder.register("weighted-boe")
class WeightedBagOfEmbeddingsEncoder(Seq2VecEncoder):
    """
    ...
    def __init__(self,
        ...
        super(WeightedBagOfEmbeddingsEncoder, self).__init__()
        self._embedding_dim = embedding_dim
        self._weight = nn.Linear(embedding_dim, 1)
    ...
    def forward(self, tokens: torch.Tensor, mask: torch.Tensor = None):  #pylint: disable=arguments-differ
        gamma = F.softmax(self._weight(tokens), dim=1)
        tokens = tokens * gamma
        if mask is not None:
