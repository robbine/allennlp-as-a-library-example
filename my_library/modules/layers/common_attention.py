...
def _generate_relative_positions_embeddings(length,
    ...
    embeddings = torch.index_select(embedding, 0,
                                    relative_positions_matrix.view(-1))
...
def dot_product_attention_relative(q, k, v, bias, max_relative_position,
                                   dropout, key_embedding, value_embedding):
    ...
    relations_keys = _generate_relative_positions_embeddings(
        length, depth, max_relative_position, key_embedding, device)
    relations_values = _generate_relative_positions_embeddings(
        length, depth, max_relative_position, value_embedding, device)
...
def _make_local_block(x, depth, batch, heads, num_blocks, block_length):
    ...
    return local_block.view(batch, heads, num_blocks - 1, block_length * 2,
                            depth)


def masked_local_attention_1d(q, k, v, block_length=64, dropout=None):
    ...
    tail_output = dot_product_attention(tail_q, local_k, local_v, bias,
                                        dropout)
    tail_output = tail_output.view(batch, heads,
                                   (num_blocks - 1) * block_length, depth_v)
...
def get_relative_embeddings_left(relative_embeddings, max_relative_position,
                                 length, depth, num_heads,
    ...
    if heads_share_relative_embedding:
        used_relative_embeddings = padded_relative_embeddings.narrow(
            0, start_slice_position, length)
    else:
        used_relative_embeddings = padded_relative_embeddings.narrow(
            1, start_slice_position, length)
...
def masked_relative_local_attention_1d(q,
    ...
    first_rel_embeddings = get_relative_embeddings_left(
        relative_key_embeddings, rel_embed_length, block_length, depth_k,
    ...
    rel_embeddings = get_relative_embeddings_left(
        relative_key_embeddings, rel_embed_length, 2 * block_length, depth_k,
        heads, heads_share_relative_embedding)
    rel_logits = matmul_with_relative_keys(rel_tail_q, rel_embeddings,
                                           heads_share_relative_embedding)
    ...
    all_logits = (
        torch.matmul(rel_tail_q,
                     rel_k.transpose(-1, -2).contiguous()) + all_rel_logits)
    ...
    if add_relative_to_values:
        ...
        weights_for_unmasked, weights_for_masked = (torch.split(
            weights, local_length // 2, dim=3))
        ...
        value_rel_embeddings_unmasked = get_relative_embeddings_left(
            relative_value_embeddings, rel_embed_length, 2 * block_length,
            depth_v, heads, heads_share_relative_embedding)
        # The unmasked part starts with index -1 as opposed 0 has take uptil last.
        if heads_share_relative_embedding:
            value_rel_embeddings_unmasked = value_rel_embeddings_unmasked[:
                                                                          -1, :]
        else:
            value_rel_embeddings_unmasked = value_rel_embeddings_unmasked[:, :
                                                                          -1, :]
        value_rel_embeddings_masked = get_relative_embeddings_left(
            relative_value_embeddings, rel_embed_length, block_length, depth_v,
            heads, heads_share_relative_embedding)

        # [batch (*num_blocks), heads, query length, key length]
        rel_weights = torch.cat([rel_weights_unmasked, rel_weights_masked],
                                dim=3)
        if heads_share_relative_embedding:
    ...
    output = output.view(batch, heads, (num_blocks - 1) * block_length,
                         depth_v)
...
def masked_within_block_local_attention_1d(q, k, v, block_length=64):
    ...
    attention += attention_bias_lower_triangle(block_length).view(
...
def get_relative_embeddings_left_right(
        relative_embeddings, max_relative_position, length, depth, num_heads,
        heads_share_relative_embedding):
    """Instantiate or retrieve relative embeddings, sliced according to length.
    ...
    if heads_share_relative_embedding:
        used_relative_embeddings = padded_relative_embeddings.narrow(
            0, slice_start_position, 2 * length - 1)
    else:
        used_relative_embeddings = padded_relative_embeddings.narrow(
            1, slice_start_position, 2 * length - 1)
    return used_relative_embeddings
...
def dot_product_unmasked_self_attention_relative_v2(
    ...
    key_relative_embeddings = get_relative_embeddings_left_right(
        relative_key_embeddings, max_relative_position, length, depth_k,
    ...
    if add_relative_to_values:
        ...
        value_relative_embeddings = get_relative_embeddings_left_right(
            relative_value_embeddings, max_relative_position, length, depth_v,
            num_heads, heads_share_relative_embedding)
        ret += matmul_with_relative_values(relative_weights,
...
def multihead_attention(use_fp16,
    ...
    if attention_type == "dot_product":
        ...
        x = masked_local_attention_1d(
            q, k, v, block_length=block_length, dropout=dropout)
...
def embedding_postprocessor(input_tensor,
    ...
    if use_token_type:
        ...
        token_type_embedding_res = token_type_embedding(
            token_type_ids.long()) * (input_mask_tensor.unsqueeze(-1).float())
        if use_fp16:
    ...
    if use_position_embeddings:
        ...
        position_embedding_res = position_embedding(range_tensor) * (
            input_mask_tensor.unsqueeze(-1).float())
        if use_fp16:
...
def create_attention_mask_from_input_mask(from_tensor, to_mask,
    ...
    unsqueeze_to_mask = to_mask.unsqueeze(1).float()
    ...
    broadcast_ones = utils.get_one_tensor(
        [batch_size, from_seq_length, 1],
        device=util.get_device_of(unsqueeze_to_mask))
