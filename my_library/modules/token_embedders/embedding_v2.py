...
@TokenEmbedder.register("embedding_v2")
class EmbeddingV2(TokenEmbedder):
    """
...
class EmbeddingsTextFile(Iterator[str]):
    ...
    def __init__(self,
        ...
        if zipfile.is_zipfile(main_file_local_path):  # ZIP archive
            ...
            self._handle = package.open(
                main_file_local_path, 'rt', encoding=encoding)  # type: ignore
    ...
    def _open_inside_zip(self,
        ...
        cached_archive_path = cached_path(
            archive_path, cache_dir=self._cache_dir)
    ...
    def _get_the_only_file_in_the_archive(members_list: Sequence[str],
        ...
        if len(members_list) > 1:
            raise ValueError(
                ...
                (archive_path,
                 format_embeddings_file_uri('path_or_url_to_archive',
                                            'path_inside_archive')))
